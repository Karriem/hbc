  ...
  feature("Save Caregiver") {
    ...
    scenario(" Create Tables in the Database ") {
      ...
      Database.forURL("jdbc:mysql://localhost:3306/test", driver = "com.mysql.jdbc.Driver", user = "root", password = "admin").withSession { implicit session =>
        ...
        val addressRecord = Address("30 Chester Road", "30 Chester Road" , "7700" , Some(0), Some(0), Some(0) , Some(0) , Some(id))
       // val contactRecord = Contact(Some("021798000") , "0786119726", "n@gmail.com", Some(0), Some(0), Some(id) , Some(0), Some(0))
        val demoRecord = Demographic(23 ,"Female", "1976/03/16" , Some(id), Some(0), Some(0) , Some(0) )
        ...
        def Read(age: Int, id : Long , cellNumber: String, address: String) = {
         ...
         /* contactRepo foreach { case (con: Contact) =>
            if (con.coordinatorId == Option(id)) {
        ...
        def Update(name:String, id:Long, age: Int, cellNumber: String, address: String) = {
          ...
          //contactRepo.filter(_.coordinatorId === id).map(_.cellNumber).update(cellNumber)
